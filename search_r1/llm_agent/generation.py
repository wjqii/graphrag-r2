...
class LLMGenerationManager:
    ...
    def _postprocess_responses(self, responses: torch.Tensor) -> torch.Tensor:
        ...
        # mark!!!!!!!!!!!!!!!!!
        # responses_str = [resp.split('<|end_of_query|>')[0] + '<|end_of_query|>'
        #          if '<|end_of_query|>' in resp 
        #          else resp.split('</answer>')[0] + '</answer>'
        #          if '</answer>' in resp 
        #          else resp
        #          for resp in responses_str]
        ...
        for i, (action, active) in enumerate(zip(cur_actions, active_mask)):
            ...
            if not active:
                ...
                if action == 'answer':
                    ...
                    # mark!!!!!!!!!!
                    # next_obs.append(f'\n\n<|begin_of_documents|>{search_results.pop(0).strip()}<|end_of_documents|>\n\n')
                    dones.append(0)
                    valid_action.append(1)
                    is_search.append(1)
                else:
...
# If I want to search, I should put the query between <|begin_of_query|> and <|end_of_query|>. \
    ...
    def postprocess_predictions(self, predictions: List[Any]) -> Tuple[List[int], List[bool]]:
        ...
        for prediction in predictions:
            if isinstance(prediction, str): # for llm output
                ...
                #     action = 'search'  # 设置固定的action名称
                # else:
                #     pattern = r'<(answer)>(.*?)</\1>'
                #     match = re.search(pattern, prediction, re.DOTALL)

                #     # pattern_answer = r'<answer>(.*?)</answer>'
                #     # match = re.search(pattern_answer, prediction, re.DOTALL)
                #     if match:
