...
os.environ["WANDB_API_KEY"] = 'ed908a5e001beba406c3612d998de5242ec1570b'
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, format_score=0., reward_mode='stage2') -> None:
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if self.reward_mode == 'stage1':
                # Stage 1: format_reward + retrieve_w_decay (learn search behavior)
                format_score = qa_em.compute_score_format_reward(
                ...
                f1_plus_score = qa_em.compute_score_f1_plus(
                    solution_str=sequences_str, ground_truth=ground_truth)
                format_score = qa_em.compute_score_format_punishment(
                    solution_str=sequences_str, ground_truth=ground_truth)
                score = f1_plus_score + format_score
            else:
                # original EM mode
...
def main_task(config):
    ...
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, reward_mode=config.algorithm.get('reward_mode', 'stage2'))

    # Note that we always use function-based RM for validation
    val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1, reward_mode=config.algorithm.get('reward_mode', 'stage2'))
