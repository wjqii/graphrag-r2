...
def pad_2d_list_to_length(data, pad_token_id, max_length):
    """
    Pad a list of lists to max_length and convert to a tensor.
    input: list of lists (e.g. list of token id lists)
    output: torch.Tensor of shape [batch_size, max_length]
    """
    batch_size = len(data)
    result = torch.full((batch_size, max_length), pad_token_id, dtype=torch.long)
    for i, row in enumerate(data):
        seq_len = min(len(row), max_length)
        result[i, :seq_len] = torch.tensor(row[:seq_len], dtype=torch.long)
    return result
...
def get_response_mask(response_id, eos_token, dtype):
    ...
    response_id = torch.as_tensor(response_id, dtype=torch.long)
    if response_id.dim() == 0:
        response_id = response_id.unsqueeze(0).unsqueeze(0)
    elif response_id.dim() == 1:
        response_id = response_id.unsqueeze(0)
    is_padding = response_id == eos_token
    cumsum = torch.cumsum(is_padding.long(), dim=-1)
    mask = (cumsum == 0).to(dtype)
    return mask
