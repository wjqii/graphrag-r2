...
LoRAModel = None
_LOKA_MODEL_IMPORT_ERRORS = []

try:
    from vllm.lora.models import LoRAModel
except (ImportError, ModuleNotFoundError) as e:
    _LOKA_MODEL_IMPORT_ERRORS.append(f"vllm.lora.models: {e}")

if LoRAModel is None:
    try:
        from vllm.lora.lora_model import LoRAModel
    except (ImportError, ModuleNotFoundError) as e:
        _LOKA_MODEL_IMPORT_ERRORS.append(f"vllm.lora.lora_model: {e}")

if LoRAModel is None:
    try:
        from vllm.lora import LoRAModel
    except (ImportError, ModuleNotFoundError) as e:
        _LOKA_MODEL_IMPORT_ERRORS.append(f"vllm.lora: {e}")

if LoRAModel is None:
    try:
        LoRAModel = LRUCacheWorkerLoRAManager._lora_model_cls
    except AttributeError:
        _LOKA_MODEL_IMPORT_ERRORS.append("LRUCacheWorkerLoRAManager._lora_model_cls not found")

if LoRAModel is None:
    raise ImportError(
        f"Cannot import LoRAModel from vllm. Tried paths: {'; '.join(_LOKA_MODEL_IMPORT_ERRORS)}. "
        f"Your vLLM version may have LoRAModel in a different module. "
        f"Run: python3 -c \"import vllm.lora.worker_manager as m; print(m.LRUCacheWorkerLoRAManager._lora_model_cls)\""
    )
...
class VLLMHijack:
    @staticmethod
    def hijack():
        _is_v073_plus = is_version_ge(pkg="vllm", minver="0.7.3")
            ...
            try:
                ...
                if isinstance(lora_request, TensorLoRARequest):
                    ...
                    if _is_v073_plus:
                        ...
                        model_obj = self._adapter_manager.model
                        if hasattr(model_obj, "hf_to_vllm_mapper") and model_obj.hf_to_vllm_mapper is not None:
                            hf_to_vllm_mapper = model_obj.hf_to_vllm_mapper
