...
def _get_fsdp_classes():
    """Lazily import FSDP-related classes that may not be available in older PyTorch versions."""
    from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
    try:
        from torch.distributed.fsdp import CPUOffloadPolicy, FSDPModule, MixedPrecisionPolicy
        return FSDP, FSDPModule, CPUOffloadPolicy, MixedPrecisionPolicy
    except ImportError:
        try:
            from torch.distributed.fsdp import FSDPModule
            return FSDP, FSDPModule, None, None
        except ImportError:
            from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
...
def get_init_weight_context_manager(use_meta_tensor=True, **kwargs):
...
def fsdp_version(module):
    FSDP, FSDPModule, _, _ = _get_fsdp_classes()
...
def get_fsdp_state_ctx(model, state_dict_type, state_dict_cfg, optim_cfg):
    from contextlib import nullcontext
    from torch.distributed.fsdp import FullyShardedDataParallel as FSDP

    if fsdp_version(model) == 1:
        return FSDP.state_dict_type(model, state_dict_type, state_dict_cfg, optim_cfg)
    return nullcontext()
...
def get_fsdp_full_state_dict(model, offload_to_cpu=True, rank0_only=True):
    ...
    if fsdp_version(model) == 1:
        cfg = FullStateDictConfig(offload_to_cpu=offload_to_cpu, rank0_only=rank0_only)
        with FSDP.state_dict_type(model, StateDictType.FULL_STATE_DICT, cfg):
            return model.state_dict()
    return model.state_dict()


def apply_fsdp2(module, fsdp_kwargs, fsdp_config):
    from torch.distributed._composable.fsdp import fully_shard

    fully_shard(module, **fsdp_kwargs)


def fsdp2_load_full_state_dict(module, full_state, fsdp_mesh, cpu_offload):
    module.load_state_dict(full_state)


def layered_summon_lora_params(module):
...
def load_fsdp_model_to_gpu(module):
    device_id = torch.cuda.current_device()
    load_fsdp_param_and_grad(module, device_id, load_grad=False)


def offload_fsdp_model_to_cpu(module):
    offload_fsdp_param_and_grad(module, offload_grad=False)
