...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    ...
                    # === 💥 核心魔法：只挑出名字里带 lora 的参数 ===
                    lora_dict = {k: v for k, v in model_state_dict.items() if 'lora' in k}
                    if len(lora_dict) > 0:
                        model_state_dict = lora_dict  # 替换成只有 lora 的小字典
                    # ============================================
                    
                    torch.save(model_state_dict, model_path)
                    log_with_rank(f"Saved model to {os.path.abspath(model_path)} (LoRA Params only: {len(lora_dict)>0})", rank=self.rank, logger=logger)

                if self.should_save_optimizer:
                    # === 💥 极致省流：直接把优化器也扬了（因为咱们后续主要为了拿权重推理，不需要断点续训优化器）===
                    # optimizer_state_dict = self.optimizer.state_dict()
                    # torch.save(optimizer_state_dict, optim_path)
                    log_with_rank(f"Skipped saving optimizer to save disk space!", rank=self.rank, logger=logger)
        ...
        if self.should_save_hf_model:
            ...
            # === 💥 连 HuggingFace 格式也一起瘦身 ===
            if state_dict is not None:
                lora_hf_dict = {k: v for k, v in state_dict.items() if 'lora' in k}
                if len(lora_hf_dict) > 0:
                    state_dict = lora_hf_dict
            # ========================================
            if self.rank == 0:
